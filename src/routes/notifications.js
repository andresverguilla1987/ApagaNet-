...
router.post("/notifications/subscriptions", async (req, res) => {
  try {
    const { type, endpoint_url=null, email=null, home_id=null, device_id=null, user_id=null, active=true } = req.body || {};
    if (!["webhook","email"].includes(type)) return res.status(400).json({ error: "invalid type" });
  ...
  } catch (e) { console.error("POST /notifications/subscriptions", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.get("/notifications/subscriptions", async (req, res) => {
  try {
    ...
    res.json(rows);
  } catch (e) { console.error("GET /notifications/subscriptions", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.post("/admin/notifications/dispatch", requireTask, async (_req, res) => {
  try {
    ...
    res.json(r);
  } catch (e) { console.error("POST /admin/notifications/dispatch", e); res.status(500).json({ error: "internal_error" }); }
});

...
let notify = null;
try { notify = (await import("../services/notify.js")).default || (await import("../services/notify.js")); } catch {}

const router = express.Router();
const toLimit = (v) => Math.max(1, Math.min(200, Number(v) || 50));
const toSince = (v) => { if (!v) return null; const d = new Date(v); return isNaN(d.getTime()) ? null : d.toISOString(); };
...
router.post("/", async (req, res) => {
  try {
    ...
    const q = `insert into alerts (id, user_id, home_id, device_id, level, title, message, metadata)
               values (gen_random_uuid(), null, $1, $2, $3, $4, $5, $6::jsonb) returning *`;
    const { rows } = await pool.query(q, [home_id, device_id, level, title, message, JSON.stringify(metadata)]);
    const alert = rows[0];
    if (notify?.enqueueAlert) { try { await notify.enqueueAlert(alert); } catch (e) { console.warn("[notify] enqueue", e.message); } }
    res.status(201).json(alert);
  } catch (e) { console.error("POST /alerts", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.get("/", async (req, res) => {
  try {
    ...
    res.json(rows);
  } catch (e) { console.error("GET /alerts", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.json(rows[0]);
  } catch (e) { console.error("GET /alerts/:id", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.patch("/:id/read", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`update alerts set read_at=${read ? "now()" : "null"} where id=$1 returning *`, [req.params.id]);
    if (!rows[0]) return res.status(404).json({ error: "not_found" });
    res.json(rows[0]);
  } catch (e) { console.error("PATCH /alerts/:id/read", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const newLevel = level ?? row.level; if (!["info","warn","critical"].includes(newLevel)) return res.status(400).json({ error: "invalid level" });
    const newTitle = (title ?? row.title); if (!newTitle || !String(newTitle).trim()) return res.status(400).json({ error: "title is required" });
    ...
    res.json(upd[0]);
  } catch (e) { console.error("PUT /alerts/:id", e); res.status(500).json({ error: "internal_error" }); }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (e) { console.error("DELETE /alerts/:id", e); res.status(500).json({ error: "internal_error" }); }
});

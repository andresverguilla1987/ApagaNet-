// src/routes/notifications.pro.js (ESM) — subscriptions + admin dispatch
...
function requireTaskSecret(req, res, next){
  ...
  const headerAlt =
    (req.headers["x-task-secret"] || req.headers["x-admin-secret"] || "").toString().trim();
...
router.post("/notifications/subscriptions", requireJWT, async (req,res)=>{
  try{
    ...
    const q = `
      ...
      RETURNING id;
    `;
    const r = await pool.query(q, [userId, channel, address, levels, qh_from, qh_to, timezone]);
...
router.get("/notifications/subscriptions", requireJWT, async (req,res)=>{
  try{
    ...
    const r = await pool.query(
      `SELECT id, channel, address, levels,
              quiet_from as "quietFrom", quiet_to as "quietTo", tz
      ...
      if (s.quiet_from && s.quiet_to && s.tz){
        try{
          const now = new Date().toLocaleTimeString("en-GB",
            { hour12:false, hour:"2-digit", minute:"2-digit", timeZone:s.tz }).slice(0,5);
          const a = String(s.quiet_from).slice(0,5);
          const b = String(s.quiet_to).slice(0,5);
